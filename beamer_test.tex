...
\begin{frame}\frametitle{Line Chart with little data}
	{ \centering 
	\lineChart[width=0.8\textwidth, height=0.6\textwidth, xunit=Money (in €), yunit=Date, options={"title": "Title"}]{SELECT * FROM data1;}%
	\\}
\end{frame}
...
\begin{frame}\frametitle{Line Chart with much data}
	{\centering
	    \lineChart[width=1.0\textwidth, height=0.5\textwidth, xunit=Row Number, yunit={Runtime [in sec]}]{SELECT ROWNUM(), runtime FROM fact_experiment;}%
	\\}
\end{frame}
