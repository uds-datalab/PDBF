            ...
            return {
              ...
              format: function(x){
				  if (data.intable) {
					  ...
					  var ret = "<span class=\"hasTooltip\">"+formatter(x[attr])+"</span><span style=\"position:absolute;\" ><table><tr>";
					  for (key in x) {
      ...
      try {
        ...
        refreshDelayed = (function(_this) {
          return function() {
			...
			var tmp = [];
			var dropdown = _this.find(".pvtVals select.pvtAttrDropdown");
			unusedAttrsContainer = _this.find("td.pvtUnused.pvtAxisContainer");
			$(unusedAttrsContainer).children("li").map(function(x) {
				var name = this.children[0].innerHTML.substr(0, this.children[0].innerHTML.length - '<span class="pvtTriangle"> 0</span>'.length);
				var bla = dropdown.val();
				if (name != bla) {
					tmp[tmp.length] = name;
				}
			});
			...
			$('.hasTooltip').hover(function() {
				...
				var tooltip = $(this).next('span')[0];
				setTimeout(function(o){o.style.visibility = 'hidden';}, 500, tooltip);
