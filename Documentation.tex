...
%
\noindent PDBF files are created from LaTeX code and data in SQL format. The raw data can either be a SQL statements string, a file with SQL statements, or in a database (currently PostgreSQL/MySQL/MariaDB supported). In the LaTeX code one can then specify how the PDBF element (currently charts/pivot tables/multiplot charts/sql statements are supported) is created from the raw data with options and an SQL query.\\[4pt]
%
\noindent PDBF toolkit is written in Java and LaTeX an can therefore be run on Windows/Mac/Linux. PDBF documents are also platform independent and run on any OS with a browser/PDF viewer.\\[4pt]
%
...
Attributes with spaces sourrounded by square bracets. E.g. SELECT a AS [This is a test] FROM test;\\

\noindent TODO: write about special sql functions. AlaSQL (\url{https://github.com/agershun/alasql}).\\
