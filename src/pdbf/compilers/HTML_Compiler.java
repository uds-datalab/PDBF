...
public class HTML_Compiler {
	...
	try {
	    ...
	    String viewerHEAD = FileUtils.readFileToString(new File(baseDirData
		    + "template-head-alasql.html"), Tools.utf8);
	    ...
	    String all = FileUtils.readFileToString(new File(baseDirData
		    + "all"), Tools.utf8);
	    String preload = FileUtils.readFileToString(new File(baseDir
		    + "pdbf-preload"));
	    viewer = viewerHEAD
		    + "pdf_base64 = \""
		    + Tools.encodeFileToBase64Binary(new File(pdfname
			    .substring(0, pdfname.length() - 4) + "Embed.pdf"))
		    + "\";\r\n"
		    + "db_base64 = \"\";\r\n"
		    + "json_base64 = \""
	    ...
	    String insert2 = "1337 0 obj\n" + "stream\n" + "</script>\n"
		    + viewer + "<script>\n" + "endstream\n" + "endobj\n";
	    String pdfcontent = FileUtils.readFileToString(new File(pdfname),
		    StandardCharsets.ISO_8859_1);
	    if (pdfcontent.toLowerCase().contains("</script>")) {
		System.err
			.println("The generated pdf cannot be used to generate a pdbf document!"
				+ " Try to either change some content in your tex file or try to "
				+ "add \\pdfcompresslevel=8 to your tex file.");
		System.exit(-1);
	    }
	    ...
	    FileUtils.writeStringToFile(new File(outfile), sb.toString(),
		    StandardCharsets.ISO_8859_1);
