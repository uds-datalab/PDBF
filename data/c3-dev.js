(function (window) {
    ...
    c3_chart_internal_fn.updateSizes = function () {
        ...
        $$.margin = config.axis_rotated ? {
            ...
            top: 4 * config.completeScale + $$.getCurrentPaddingTop(), // for top tick text
            right: hasArc ? 0 : $$.getCurrentPaddingRight(),
        ...
        $$.margin2 = config.axis_rotated ? {
            top: $$.margin.top,
            right: NaN,
            bottom: 20 * config.completeScale + legendHeightForBottom,
    ...
    c3_chart_internal_fn.parseDate = function (date) {
        ...
        if (!parsedDate || isNaN(+parsedDate)) {
        	//second try
            parsedDate = new Date(replaceAll(date, "-", "/"));
    ...
    c3_chart_internal_fn.getCurrentPaddingRight = function () {
        var $$ = this, config = $$.config,
            defaultPadding = 10 * config.completeScale, legendWidthOnRight = $$.isLegendRight ? $$.getLegendWidth() + 10 * config.completeScale : 0;
        if (isValue(config.padding_right)) {
            return config.padding_right + 1; // 1 is needed not to hide tick line
        } else if (config.axis_rotated) {
            return defaultPadding + legendWidthOnRight;
        } else if (!config.axis_y2_show || config.axis_y2_inner) { // && !config.axis_rotated
            return 4 * config.completeScale + legendWidthOnRight + ($$.axis.getY2AxisLabelPosition().isOuter ? 20 * config.completeScale : 0);
        } else {
            return $$.getAxisWidthByAxisId('y2') + legendWidthOnRight;
        }
    };
    ...
    c3_chart_internal_fn.getHorizontalAxisHeight = function (axisId) {
        ...
        if (axisId === 'x' && !config.axis_x_show) { return 8 * $$.config.completeScale; }
        if (axisId === 'x' && config.axis_x_height) { return config.axis_x_height; }
        if (axisId === 'y' && !config.axis_y_show) { return (config.legend_show && !$$.isLegendRight && !$$.isLegendInset ? 10 : 1) * $$.config.completeScale; }
        ...
        return h + ($$.axis.getLabelPositionById(axisId).isInner ? 0 : 13*$$.config.completeScale) + (axisId === 'y2' ? -10 : 0) + 4 * config.completeScale; //padding
    };
    ...
    c3_chart_internal_fn.getTextRect = function (text, cls, element) {
        var dummy = this.d3.select('body').append('div').classed('c3', true),
            svg = dummy.append("svg").style('visibility', 'hidden').style('position', 'fixed').style('top', 0).style('left', 0).style('font-size', $(this.config.bindto.parentElement).css('font-size')).style('font-family', $(this.config.bindto.parentElement).css('font-family')),
            rect;
        svg.selectAll('.dummy')
            .data([text])
          .enter().append('text')
    ...
    c3_chart_internal_fn.getTooltipContent = function (d, defaultTitleFormat, defaultValueFormat, color) {
        ...
        if (config.data_groups.length === 0) {
            d.sort(function(a,b){
                return typeof a === "undefined" || typeof b === "undefined" ? orderAsc ? a.value - b.value : b.value - a.value : 0;
            });
        } else {
    ...
    c3_chart_internal_fn.updateSizeForLegend = function (legendHeight, legendWidth) {
        var $$ = this, config = $$.config, insetLegendPosition = {
            top: $$.isLegendTop ? $$.getCurrentPaddingTop() + (config.legend_inset_y + 5.5) * config.completeScale : $$.currentHeight - legendHeight - $$.getCurrentPaddingBottom() - config.legend_inset_y * config.completeScale,
            left: $$.isLegendLeft ? $$.getCurrentPaddingLeft() + (config.legend_inset_x + 0.5) * config.completeScale : $$.currentWidth - legendWidth - $$.getCurrentPaddingRight() - (config.legend_inset_x + 0.5) * config.completeScale
        };

        $$.margin3 = {
            top: ($$.isLegendRight ? 0 : $$.isLegendInset ? insetLegendPosition.top : $$.currentHeight - legendHeight + 6 * config.completeScale),
            right: NaN,
            bottom: 0,
    ...
    c3_chart_internal_fn.updateLegend = function (targetIds, options, transitions) {
        ...
        function updatePositions(textElement, id, index) {
            ...
            function updateValues(id, withoutStep) {
                ...
                steps[id] = step;
                margins[step] = $$.isLegendInset ? 10 * $$.config.completeScale : margin;
        ...
        if ($$.isLegendRight) {
            ...
            xForLegend = function (id) { return maxWidth * steps[id] + 10 * config.completeScale; };
            yForLegend = function (id) { return margins[steps[id]] + offsets[id]; };
        } else {
            xForLegend = function (id) { return margins[steps[id]] + offsets[id]; };
            yForLegend = function (id) { return maxHeight * steps[id]; };
        }
        xForLegendText = function (id, i) { return xForLegend(id, i) + (config.legend_item_tile_width) * config.completeScale; };
        yForLegendText = function (id, i) { return yForLegend(id, i) + 9 * config.completeScale; };
        xForLegendRect = function (id, i) { return xForLegend(id, i); };
        yForLegendRect = function (id, i) { return yForLegend(id, i) - 5 * config.completeScale; };
        x1ForLegendTile = function (id, i) { return xForLegend(id, i) - 2 * config.completeScale; };
        x2ForLegendTile = function (id, i) { return xForLegend(id, i) - 2 * config.completeScale + config.legend_item_tile_width * config.completeScale; };
        yForLegendTile = function (id, i) { return yForLegend(id, i) + 4 * config.completeScale; };
        ...
        l.append('rect')
            ...
            .style('fill-opacity', 0)
            .attr('x', $$.isLegendRight || $$.isLegendInset ? xForLegendRect : -200 * config.completeScale)
            .attr('y', $$.isLegendRight || $$.isLegendInset ? -200 * config.completeScale : yForLegendRect);
        l.append('line')
            .attr('class', CLASS.legendItemTile)
            .style('stroke', $$.color)
            .style("pointer-events", "none")
            .attr('x1', $$.isLegendRight || $$.isLegendInset ? x1ForLegendTile : -200 * config.completeScale)
            .attr('y1', $$.isLegendRight || $$.isLegendInset ? -200 * config.completeScale : yForLegendTile)
            .attr('x2', $$.isLegendRight || $$.isLegendInset ? x2ForLegendTile : -200 * config.completeScale)
            .attr('y2', $$.isLegendRight || $$.isLegendInset ? -200 * config.completeScale : yForLegendTile)
            .attr('stroke-width', config.legend_item_tile_height * config.completeScale);
        ...
        if (background) {
            (withTransition ? background.transition() : background)
                ...
                .attr('width', maxWidth * (step + 1) + 10 * config.completeScale);
        }
    ...
    Axis.prototype.getXAxisTickFormat = function getXAxisTickFormat() {
        var $$ = this.owner, config = $$.config,
            format = $$.isTimeSeries() ? $$.defaultAxisTimeFormat : $$.isCategorized() ? $$.categoryName : function (v) { return parseFloat(v.toFixed(10)); };
    ...
    Axis.prototype.getMaxTickWidth = function getMaxTickWidth(id, withoutRecompute) {
        ...
        if ($$.svg) {
            ...
            svg = dummy.append("svg").style('visibility', 'hidden').style('position', 'fixed').style('top', 0).style('left', 0).style('font-size', $(config.bindto.parentElement).css('font-size')).style('font-family', $(config.bindto.parentElement).css('font-family')),
        ...
        function axis(g) {
            g.each(function () {
                ...
                function tspanDy(d, i) {
                    ...
                    if (i === 0) {
                        if (orient === 'left' || orient === 'right') {
                            dy = -((counts[d.index] - 1) * (sizeFor1Char.h / 2) - 5.5*params.$$.config.completeScale);
                        } else {
                            dy = ".71em";
                        }
                    }
                ...
                function dxForText(rotate) {
                    ...
                    return tickLength * Math.sin(Math.PI * (rotate / 180));
                }
